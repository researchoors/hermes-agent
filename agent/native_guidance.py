...
ARTIFACT_GUIDANCE = (
    ...
    "declare buttons in its `actions` array that invoke server-side handlers "
    "when the user clicks them in the app. Three tiers:\n"
    "- Tier 0 (local): the client mutates its own copy, no gateway call.\n"
    "- Tier 1 (registered handler): deterministic Python in "
    "~/.hermes/plugins/actions/*.py — use this for anything writable in "
    "advance (API calls, DB writes, deletes).\n"
    "- Tier 2 (agent.prompt): routes back through a fresh agent turn — only "
    "when the action needs reasoning that can't be encoded ahead of time.\n"
    "\n"
    "Declare a button by adding to the artifact's `actions` array:\n"
    ...
    "`data-hermes-entity` is the row's key-field value. The gateway resolves "
    "the handler from the binding id; a destructive role forces a native "
    "confirmation dialog that leads with the trusted intent name before the "
    "handler runs. Security rule for handlers: treat `entity_ref` as a lookup "
    "key into the pinned artifact content and read external IDs from the "
    "stored row — never call an external API with the raw client string.\n"
    "\n"
